  ...
  return (
    <div>
      ...
      <div style={{ maxWidth:'900px', margin:'0 auto', padding:'40px 16px', display:'grid', gap:'32px' }} className="contact-grid">
        ...
        <div>
          <h2 style={{ fontWeight:700, fontSize:'17px', color:'#1a1a1a', marginBottom:'20px' }}>Contact Information</h2>
          {[
        ...
        <div style={{ background:'white', borderRadius:'8px', boxShadow:'0 0 0 1px rgba(0,0,0,0.08), 0 4px 12px rgba(0,0,0,0.06)', padding:'24px' }}>
          {sent ? (
            <div style={{ textAlign:'center', padding:'40px 0' }}>
              ...
              <h3 style={{ fontWeight:700, color:BLUE, fontSize:'18px' }}>Message Sent!</h3>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px' }}>
                <div>
                  ...
                  <input required value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))}
                    style={inp} onFocus={e=>e.target.style.borderColor=BLUE} onBlur={e=>e.target.style.borderColor='#ddd'} placeholder="Your name" />
                </div>
                <div>
                  <label style={{ display:'block', fontSize:'13px', fontWeight:500, color:'#333', marginBottom:'5px' }}>Email</label>
                  <input type="email" required value={form.email} onChange={e=>setForm(f=>({...f,email:e.target.value}))}
                    style={inp} onFocus={e=>e.target.style.borderColor=BLUE} onBlur={e=>e.target.style.borderColor='#ddd'} placeholder="you@email.com" />
                </div>
              </div>
              <div>
                ...
                <input required value={form.subject} onChange={e=>setForm(f=>({...f,subject:e.target.value}))}
                  style={inp} onFocus={e=>e.target.style.borderColor=BLUE} onBlur={e=>e.target.style.borderColor='#ddd'} placeholder="How can we help?" />
              </div>
              <div>
